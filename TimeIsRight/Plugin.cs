...
//        RacingApi.CrossedFinishLine += removeSelf;
    ...
    {
        if (timeIsRightLobby) endTournament();
    }
...
//        }
    ...
    private void setServerMessage()
    {
        ...
        {
            string hardcoreStr = Plugin.ConfigHardcore.Value ? " HARDCORE" : "";
            ChatApi.SendMessage($"/servermessage white {ZeepkistNetwork.CurrentLobby.RoundTime} The Time Is Right{hardcoreStr}<br>Be as close to, but slower than: {targetTime(Plugin.ConfigMedal.Value).GetFormattedTime()} ({Plugin.ConfigMedal.Value})");
        }
...
//        timeIsRightSolo = false;
    ...
    private static void removePlayers()
    {
        ...
        {
            float timeDelta = targetTime(Plugin.ConfigMedal.Value) - player.Time;
            ...
            if (!shouldRemove)
            {
                ...
                foreach (LeaderboardItem oldLeaderboard in leaderboard)
                {
                    ...
                    if (oldLeaderboard.Time >= targetTime(Plugin.ConfigMedal.Value)) ZeepkistNetwork.CustomLeaderBoard_SetPlayerTimeOnLeaderboard(player.SteamID, oldLeaderboard.Time, false);
                    else shouldRemove = true;
                    break;
                }
                if (!foundPlayer) shouldRemove = true;
            }
            if (shouldRemove) ZeepkistNetwork.CustomLeaderBoard_RemovePlayerFromLeaderboard(player.SteamID, false);
            ZeepkistNetwork.SendCustomChatMessage(false, player.SteamID, $"You were too fast -{timeDelta.ToString("#.000")}. Try again :zaagbladpadrood2:", "TIME IS RIGHT");
        }
    }
